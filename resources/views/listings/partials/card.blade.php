...
<a href="{{ route('listings.show', $listing->slug) }}" class="group cursor-pointer block">
    <div class="relative h-52 md:h-64 overflow-hidden rounded-xl mb-3 md:mb-4">
        <img src="{{ $image }}" alt="{{ $listing->name }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500"/>
        @if($listing->featured)
            <div class="absolute top-3 left-3 md:top-4 md:left-4 bg-primary text-white px-2 py-1 rounded text-xs font-bold">Featured</div>
        @endif
        <div class="absolute top-3 right-3 md:top-4 md:right-4 bg-white px-2 py-1 rounded text-xs font-bold text-navy flex items-center shadow-sm">
    ...
    <h3 class="text-base md:text-lg font-bold text-navy group-hover:text-primary transition">{{ $listing->name }}</h3>
    <p class="text-sm text-gray-500 mb-1 md:mb-2">{{ $location }}</p>
    <p class="text-primary font-extrabold text-sm md:text-base">{{ $price }}</p>
</a>
