...
<div class="bg-white border border-outline-variant/60 rounded-2xl overflow-hidden card-lift">
    <x-admin.filters-bar :action="route('admin.listings.index')">
    ...
    @else
        <form method="POST" action="{{ route('admin.listings.bulk-delete') }}">
            @csrf
            @include('admin.partials.bulk-toolbar', [
                'bulkAction' => route('admin.listings.bulk-delete'),
                'paginator' => $listings,
            ])
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <a href="{{ route('listings.show', $listing) }}" target="_blank"
                                           class="font-inter text-sm font-medium text-[#1c2c40] truncate hover:text-primary-container hover:underline block">
                                            {{ $listing->name }}
                                        </a>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="inline-flex items-center justify-end gap-1">
                                    ...
                                    @if($listing->status === 'pending')
                                        <button type="submit"
                                                formaction="{{ route('admin.listings.approve', $listing->slug) }}"
                                                formmethod="POST"
                                                class="w-9 h-9 inline-flex items-center justify-center rounded-lg hover:bg-green-50 text-green-600 hover:text-green-700 transition-colors"
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
        <div class="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto pointer-events-auto shadow-2xl">
            ...
            <div class="p-6">
                ...
                <div id="listing-modal-actions" class="flex flex-col gap-3"></div>
...
function openListingModal(data) {
        ...
        actions.innerHTML += `
            <div class="flex gap-3">
                <form method="POST" action="${data.approveUrl}" class="flex-1">
                    ...
                    <button type="submit" class="w-full py-2.5 rounded-lg bg-green-600 text-white font-inter text-sm font-semibold hover:bg-green-700">Approve Listing</button>
                </form>
    ...
    const listingUrl = data.listingUrl || (data.slug ? '/listings/' + data.slug : null);
    if (listingUrl) {
        actions.innerHTML += `<a href="${listingUrl}" target="_blank" class="flex-1 text-center py-2.5 rounded-lg bg-[#1c2c40] text-white font-inter text-sm font-semibold hover:bg-[#2a3d56]">View Listing Page</a>`;
    }
